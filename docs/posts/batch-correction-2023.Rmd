+++
title = "Adjusting batch effect in pseudobulk samples is a causal inference problem"
date = "2023-06-19"
draft = false
...
txtplot(.delta.db[,1], W.true[,1], width=50, height=20,
        xlab="estimated delta (batch 1)",
        ylab="true delta effect")

cat("batch #2\n")
txtplot(.delta.db[,2], W.true[,2], width=50, height=20,
...
cat("```\n")
cat("batch #1\n")
txtplot(.delta.db[,1], y.true[,1], xlab="estimated delta 1", ylab="true y mean 1")
cat("batch #2\n")
txtplot(.delta.db[,2], y.true[,2], xlab="estimated delta 2", ylab="true y mean 2")
cat("```\n")
```
...
cat("```\n")
cat("batch #1\n")
txtplot(log1p(ybar[,1]), y.true[,1], xlab="log1p sample mean", ylab="true y mean #1")
cat("batch #2\n")
txtplot(log1p(ybar[,2]), y.true[,2], xlab="log1p sample mean", ylab="true y mean #2")
cat("```\n")
```
...
cat("```\n")
cat("batch #1\n")
txtplot(log1p(ybar.adj[,1]), y.true[,1], xlab="adjusted sample mean", ylab="true y mean")
cat("batch #2\n")
txtplot(log1p(ybar.adj[,2]), y.true[,2], xlab="adjusted sample mean", ylab="true y mean")
cat("```\n")
```
...
cat("before batch adjustment\n")
txtplot(.tsne[,1], .tsne[,2], xlab = "tsne1", ylab = "tsne2", width=50, height=20)

.tsne <- Rtsne::Rtsne(log(1 + t(yy/.delta.db[,batch])), num_threads=4)$Y
cat("after batch adjustment\n")
txtplot(.tsne[,1], .tsne[,2], xlab = "tsne1 adj", ylab = "tsne2 adj", width=50, height=20)
cat("```\n")
```
